...
def generate_assignmnet(plan, n_students, distribution):
    ...
    path_subjects = {
        p: set(plan[plan["specialization"] == p]["subject"]) for p in paths
    }

    df = pd.DataFrame(
        "", index=range(n_students), columns=["student_id", "path"] + subjects
    )
...
def main():
    ...
    parser.add_argument(
        "-p", "--path_distribution", nargs="+", type=float, default=[0.45, 0.45, 0.1]
    )
    parser.add_argument("-o", "--out")

    args = parser.parse_args("data/schedules/schedule_1.csv -n 220 -p 90 90 30".split())
    ...
    out_path = (
        Path(args.out)
        if args.out
        else Path(f"data/assignments/subject_assignment_{plan_number}.csv")
    )
